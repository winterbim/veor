...
test('unknown tool is denied before downstream',async t=>{const c=start();t.after(()=>stop(c));await init(c);const r=await c.rpc('tools/call',{name:'surprise.delete_everything',arguments:{}});assert.equal(r.result.structuredContent.veor.decision,'DENY');assert.deepEqual(marks(c),[]);});


test('gateway consumes a signed REVIEW approval exactly once',async t=>{
 const keys=generateApprovalAuthority();const c=start({approvalKeys:keys});t.after(()=>stop(c));await init(c);const args={path:path.join(c.out,'approved.txt'),text:'x'};
